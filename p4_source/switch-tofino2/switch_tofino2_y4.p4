...
const bit<32> IPV6_LPM_TABLE_SIZE = 4096;
const bit<32> IPV6_LPM64_TABLE_SIZE = 40*1024;

// ECMP/Nexthop
const bit<32> ECMP_GROUP_TABLE_SIZE = 2048;
...
#define switch_tunnel_nexthop_width 12
const bit<32> TUNNEL_NEXTHOP_TABLE_SIZE = 4096;

// Tunnels
#define switch_tunnel_index_width 12
...
@pa_container_size("ingress", "hdr.arp.target_proto_addr", 32)
control SwitchIngress(
    ...
    apply {
        ...
        // Rotate hash by the specified amount
        rotate_hash.apply(local_md);

        // If crc16 is used as algorithm, copy calculated crc to upper hash word
        if (local_md.hash[31:16]==0) {
            local_md.hash[31:16] = local_md.hash[15:0];
        }
...
//    EgressLOU() lou;
    ...
    EgressPortStats() port_stats;

    apply {
        local_md.timestamp = eg_intr_md_from_prsr.global_tstamp[31:0];
