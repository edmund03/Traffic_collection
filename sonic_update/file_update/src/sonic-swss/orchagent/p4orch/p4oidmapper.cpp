...
// bool P4OidMapper::setUdfOID(_In_ UdfTableType object_type , _In_ std::string &key, _In_ std::string Uid, 
//                             _In_ uint32_t ref_count)
// {
//     SWSS_LOG_ENTER();

//     if (m_uidTables[object_type].find(key) != m_oidTables[object_type].end())
//     {
//         SWSS_LOG_ERROR("Key %s with SAI object type %d already exists in centralized mapper", key.c_str(), object_type);
//         return false;
//     }

//     m_uidTables[object_type][key] = {Uid, ref_count};
//     m_table.hset("", convertToDBField(object_type, key), Uid);
//     return true;
// }
...
// bool P4OidMapper::getUdfOID(_In_ UdfTableType object_type, _In_ const std::string &key, _Out_ std::string *oid)
// {
//     SWSS_LOG_ENTER();
