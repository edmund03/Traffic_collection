...
struct P4UdfRule
{
    // sai_object_id_t acl_table_oid;
    // sai_object_id_t acl_entry_oid;
    std::string udf_table_name;
    std::string udf_rule_key;
    std::string db_key;

    sai_uint32_t priority;
    std::string p4_action;
    std::map<std::string, std::string> match_fvs;
    std::map<std::string, std::string> action_fvs;
    // P4AclMeter meter;
    // P4AclCounter counter;
};
...
struct UdfMatchField
{
    uint32_t bitwidth;
    Format_UDF format;
    Match_Type_UDF match_type;
};
/*end*/
struct P4UdfField
{
...
struct P4UdfTableDefinition
{
    ...
    std::map<std::string, UdfMatchField> udf_match_field_lookup;
    std::map<std::string, std::vector<UdfActionWithParam>> rule_action_field_lookup;

    P4UdfTableDefinition() = default;
    P4UdfTableDefinition(const std::string &udf_table_name, const uint32_t priority,const uint32_t size)
        : udf_table_name(udf_table_name), priority(priority), size(size){};
};
//end
...
//added by hs
using P4UdfRuleTables = std::map<std::string, std::map<std::string, P4UdfRule>>;
using P4UdfTableDefinitions = std::map<std::string, P4UdfTableDefinition>;
//end
...
static const acl_table_attr_format_lookup_t aclMatchTableAttrFormatLookup = {
    ...
    {SAI_ACL_TABLE_ATTR_FIELD_ACL_IP_FRAG, Format::HEX_STRING},//changed by hs
