  ...
  for (const auto &ir_match : ir_table_entry.matches()) {
    ...
    switch (ir_match_info->match_field().match_type()) {
      ...
      case MatchField::RANGE: {
        const absl::StatusOr<google::protobuf::Message *> &pd_range =
            GetMutableMessage(pd_match, ir_match.name());
        if (!pd_range.ok()) {
          invalid_match_reasons.push_back(
              absl::StrCat(kNewBullet, pd_range.status().message()));
          break;
        }
        const absl::StatusOr<std::string> &pd_low = IrValueToFormattedString(
            ir_match.range().low(), ir_match_info->format());
        if (!pd_low.ok()) {
          invalid_match_reasons.push_back(
              absl::StrCat(kNewBullet, pd_low.status().message()));
          break;
        }
        const auto &low_status =
            SetStringField(*pd_range, "low", *pd_low);
        if (!low_status.ok()) {
          invalid_match_reasons.push_back(
              absl::StrCat(kNewBullet, low_status.message()));
          break;
        }
        const absl::StatusOr<std::string> &pd_high = IrValueToFormattedString(
            ir_match.range().high(), ir_match_info->format());
        if (!pd_high.ok()) {
          invalid_match_reasons.push_back(
              absl::StrCat(kNewBullet, pd_high.status().message()));
          break;
        }
        const auto &high_status = SetStringField(*pd_range, "high", *pd_high);
        if (!high_status.ok()) {
          invalid_match_reasons.push_back(
              absl::StrCat(kNewBullet, high_status.message()));
          break;
        }
        break;
      }
...
static absl::Status PdMatchEntryToIr(const IrTableDefinition &ir_table_info,
  ...
  for (const auto &[_, pd_match_name] : matches) {
    ...
    switch (ir_match_info->match_field().match_type()) {
      ...
      case MatchField::RANGE: {
        auto *ir_range = ir_match->mutable_range();
        const auto &pd_range = GetMessageField(pd_match, pd_match_name);
        if (!pd_range.ok()) {
          invalid_match_reasons.push_back(
              absl::StrCat(kNewBullet, pd_range.status().message()));
          break;
        ...
        const absl::StatusOr<std::string> &pd_low =
            GetStringField(**pd_range, "low");
        if (!pd_low.ok()) {
          invalid_match_reasons.push_back(
              absl::StrCat(kNewBullet, pd_low.status().message()));
          break;
        }
        const absl::StatusOr<IrValue> &ir_low =
            FormattedStringToIrValue(*pd_low, ir_match_info->format());
        if (!ir_low.ok()) {
          invalid_match_reasons.push_back(
              absl::StrCat(kNewBullet, ir_low.status().message()));
          break;
        }
        *ir_range->mutable_low() = *ir_low;

        const absl::StatusOr<std::string> &pd_high =
            GetStringField(**pd_range, "high");
        if (!pd_high.ok()) {
          invalid_match_reasons.push_back(
              absl::StrCat(kNewBullet, pd_high.status().message()));
          break;
        }
        const absl::StatusOr<IrValue> &ir_high =
            FormattedStringToIrValue(*pd_high, ir_match_info->format());
        if (!ir_high.ok()) {
          invalid_match_reasons.push_back(
              absl::StrCat(kNewBullet, ir_high.status().message()));
          break;
        }
        *ir_range->mutable_high() = *ir_high;
        break;
      }
      default:
        invalid_match_reasons.push_back(
