...
StatusOr<IrMatch> PiMatchFieldToIr(
  ...
  switch (match_field.match_type()) {
    ...
    case MatchField::RANGE: {
      if (!pi_match.has_range()) {
        invalid_reasons.push_back(
            absl::StrCat(kNewBullet, "Must be a range match type."));
        break;
      }

      /*TODO BY HS:*/
      const absl::StatusOr<std::string> &low =
          ArbitraryToNormalizedByteString(pi_match.range().low(), bitwidth);
      if (!low.ok()) {
        invalid_reasons.push_back(
            absl::StrCat(kNewBullet, low.status().message()));
        break;
      }
      const absl::StatusOr<std::string> &high =
          ArbitraryToNormalizedByteString(pi_match.range().high(), bitwidth);
      if (!high.ok()) {
        invalid_reasons.push_back(
            absl::StrCat(kNewBullet, high.status().message()));
        break;
      }
      ...
      if (*low > *high) {
        invalid_reasons.push_back(absl::StrCat(
            kNewBullet, "Range value has low bits that are set. low: ",
            absl::CEscape(*low),"high: ", absl::CEscape(*high)));
        break;
      }

      const absl::StatusOr<IrValue> &ir_high = ArbitraryByteStringToIrValue(
          ir_match_definition.format(), bitwidth, *high);
      if (!ir_high.ok()) {
        invalid_reasons.push_back(
            absl::StrCat(kNewBullet, ir_high.status().message()));
        break;
      }
      *match_entry.mutable_range()->mutable_high() = *ir_high;
      const absl::StatusOr<IrValue> &ir_low = ArbitraryByteStringToIrValue(
          ir_match_definition.format(), bitwidth, *low);
      if (!ir_low.ok()) {
        invalid_reasons.push_back(
            absl::StrCat(kNewBullet, ir_low.status().message()));
        break;
      }
      *match_entry.mutable_range()->mutable_low() = *ir_low;
      break;
...
StatusOr<p4::v1::FieldMatch> IrMatchFieldToPi(
  ...
  switch (match_field.match_type()) {
    ...
    case MatchField::RANGE:{
      if (!ir_match.has_range()) {
        invalid_reasons.push_back(
            absl::StrCat(kNewBullet, "Must be a range match type."));
        break;
      }
      const absl::Status &valid_low = ValidateIrValueFormat(
          ir_match.range().low(), ir_match_definition.format());
      if (!valid_low.ok()) {
        invalid_reasons.push_back(
            absl::StrCat(kNewBullet, valid_low.message()));
        break;
      }
      const absl::Status &valid_high = ValidateIrValueFormat(
          ir_match.range().high(), ir_match_definition.format());
      if (!valid_high.ok()) {
        invalid_reasons.push_back(
            absl::StrCat(kNewBullet, valid_high.message()));
        break;
      }
      const absl::StatusOr<std::string> &low = IrValueToNormalizedByteString(
          ir_match.range().low(),
          ir_match_definition.match_field().bitwidth());
      if (!low.ok()) {
        invalid_reasons.push_back(
      ...
      const absl::StatusOr<std::string> &high = IrValueToNormalizedByteString(
          ir_match.range().high(),
          ir_match_definition.match_field().bitwidth());
      if (!high.ok()) {
        invalid_reasons.push_back(
      ...
      match_entry.set_field_id(match_field.id());
       if (*low > *high) {
        invalid_reasons.push_back(absl::StrCat(
            kNewBullet, "Range value has low bits that are set. low: ",
            ir_match.range().low().DebugString(),
            "high: ", ir_match.range().high().DebugString()));
        break;
      }
      match_entry.mutable_range()->set_low(
          ArbitraryToCanonicalByteString(*low));
      match_entry.mutable_range()->set_high(
          ArbitraryToCanonicalByteString(*high));
      break;
    }
    default:
      invalid_reasons.push_back(absl::StrCat(
...
StatusOr<IrTableEntry> PiTableEntryToIr(const IrP4Info &info,
  ...
  if (table->requires_priority()) {
    if (pi.priority() <= 0) {
      invalid_reasons.push_back(
          absl::StrCat(kNewBullet,
                       "Table entries with ternary or optional or range matches require "
    ...
    invalid_reasons.push_back(
        absl::StrCat(kNewBullet,
                     "Table entries with no ternary or optional or range matches cannot "
                     "have a priority. Got ",
                     pi.priority(), " instead."));
  }
...
StatusOr<p4::v1::TableEntry> IrTableEntryToPi(const IrP4Info &info,
  ...
  if (table->requires_priority()) {
    ...
    invalid_reasons.push_back(
        absl::StrCat(kNewBullet,
                     "Table entries with no ternary or optional or range "
