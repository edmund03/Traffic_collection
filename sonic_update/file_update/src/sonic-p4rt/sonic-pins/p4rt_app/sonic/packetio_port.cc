 ...
 public:
  ...
  uint64_t readData() override {
    ...
    if (msg_len < 0) {
      ...
      LOG(INFO) << "read data successs:" << read_buffer_;
...
DiscoverPacketIoPorts(const SystemCallAdapter &system_call_adapter) {
  ...
  //add port that we dont need
  std::vector<std::string> PortNoNeed;
  for(int i = 128; i <= 240; i += 1)
  PortNoNeed.push_back(std::string("Ethernet" + std::to_string(i)));
  ...
  for (intf = head; intf != nullptr; intf = intf->ifa_next) {
    ...
    //Ethernet224 & Ethernet228 is what we dont need to get lldp
    auto result = std::find(PortNoNeed.begin(),PortNoNeed.end(),intf->ifa_name);
    if(result != PortNoNeed.end())
        continue;
...
std::thread StartReceive(
  ...
  LOG(INFO) << "std::thread StartReceive";
