...
int ProcessReceiveMessageCallback(struct nl_msg* msg, void* arg) {
  ...
  LOG(INFO) << "ProcessReceiveMessage";
  if (!status.ok()) {
...
absl::StatusOr<std::thread> StartReceive(
  ...
  LOG(INFO) << "<std::thread> StartReceive start";
