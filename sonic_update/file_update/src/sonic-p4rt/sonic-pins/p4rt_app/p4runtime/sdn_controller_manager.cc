...
bool SdnControllerManager::SendStreamMessageToPrimary(
    ...
    const p4::v1::StreamMessageResponse& response) {
  absl::MutexLock l(&lock_);
  bool rootconnection = false;
  // Get the primary election ID for the controller role.
  absl::optional<absl::uint128> election_id_past_for_role =
      election_id_past_by_role_[role_name];

  absl::optional<std::string> role_name_root = std::nullopt;
  absl::optional<absl::uint128> election_id_past_for_role_root = election_id_past_by_role_[role_name_root];

   //LOG(INFO) << "judge if there is primary connection";
  // If there is no election ID set, then there is no primary connection.
  if (!election_id_past_for_role.has_value() && !election_id_past_for_role_root.has_value()) return false;

  //LOG(INFO) << "get primary connection";

  if(election_id_past_for_role_root.has_value()) {
    election_id_past_for_role = election_id_past_for_role_root;
    rootconnection = true;
    //LOG(INFO) << "get root connection";
  }

  // Otherwise find the primary connection.
  SdnConnection* primary_connection = nullptr;
  for (const auto& connection : connections_) {
    if (!rootconnection && connection->GetRoleName() == role_name &&
        connection->GetElectionId() == election_id_past_for_role) {
      primary_connection = connection;
    } 

    if(rootconnection && connection->GetElectionId() == election_id_past_for_role 
        && !(connection->GetRoleName()).has_value()) {
        primary_connection = connection;
        break;
    }
