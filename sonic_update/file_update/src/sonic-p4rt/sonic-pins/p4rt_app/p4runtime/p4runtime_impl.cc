...
absl::Status AppendTableEntryReads(
  ...
  for (const auto& app_db_key :
    ...
    LOG(INFO) << "READ TABLE:" << ir_table_entry.table_name();
    ...
    LOG(INFO) << "table id:" << (*translate_status).table_id();
    LOG(INFO) << "priority:" << (*translate_status).priority();
  ...
  for (const auto& meta : packet.metadata()) {
    switch (meta.metadata_id()) {
      case PACKET_OUT_EGRESS_PORT_ID: {
        //LOG(INFO)<<" send packetout PACKET_OUT_EGRESS_PORT_ID :"<< egress_port_id.c_str();
        egress_port_id = meta.value();
        break;
      }
      case PACKET_OUT_SUBMIT_TO_INGRESS_ID: {
        //LOG(INFO)<<" send packetout PACKET_OUT_SUBMIT_TO_INGRESS_ID :";
        ASSIGN_OR_RETURN(
            submit_to_ingress,
      ...
      case PACKET_OUT_UNUSED_PAD_ID: {
        //LOG(INFO)<<" send packetout PACKET_OUT_UNUSED_PAD_ID :";
        // Nothing to do.
        break;
      }
      default:
        LOG(INFO)<<" send packetout error :";
...
grpc::Status P4RuntimeImpl::Write(grpc::ServerContext* context,
  ...
  try {
    ...
    LOG(INFO) << "get write request from ons";
    ...
    LOG(INFO) << "ready to insert APPDB";
...
grpc::Status P4RuntimeImpl::Read(
  ...
  try {
    ...
    LOG(INFO) << "return grpc ok";
    //LOG(INFO) << "value:" << response_status.value().ToString();
...
absl::StatusOr<std::thread> P4RuntimeImpl::StartReceive(bool use_genetlink) {
  ...
  auto SendPacketInToController =
    ...
    *response.mutable_packet()->mutable_payload() = payload;
    ...
    bool result;
    //LOG(INFO) << "send packet_in data..." << payload.c_str();

    result = controller_manager_->SendStreamMessageToPrimary(
        P4RUNTIME_ROLE_SDN_CONTROLLER, response);

    LOG(INFO) << "send packet_in data..." << result;
